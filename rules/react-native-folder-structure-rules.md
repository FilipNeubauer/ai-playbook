...
This document defines the folder structure for React Native Expo applications. Follow this structure when creating new files, features, or components. When a folder is not needed for the current scope, omit it — do not create empty directories.
...
- Route files are thin — import and render from `features/`.

### Features (`features/`)

One directory per feature. Kebab-case names (e.g., `home`, `patient-detail`, `settings`).

| Folder        | Purpose                                                               |
| ------------- | --------------------------------------------------------------------- |
| `components/` | Feature components. PascalCase. Sub-directories when large.           |
| `hooks/`      | Feature hooks. Wrap API calls, return clean interfaces.               |
...
For simple features, a single component file without subdirectories is fine.
...
- **`hooks/`** — Cross-cutting hooks only. Feature-specific hooks live in `features/<feature>/hooks/`.
...
- **Route files are thin.** `app/` route files import and render feature components — no business logic.
- **Named exports** for reusable components. Default exports only for Expo Router convention files.
- **Create folders only when needed.** Simple features can be a single component file.
